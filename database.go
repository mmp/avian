...
func InitializeStaticDatabase(dbChan chan *StaticDatabase) {
	...
	if pos == nil || db.LoadSectorFile(pos.SectorFile) != nil {
	...
	if pos == nil || db.LoadPositionFile(pos.PositionFile) != nil {
...
var (
	...
	//go:embed resources/airports.csv.zst
	airportsRaw string
...
func parseAirports() map[string]Airport {
	...
	mungeCSV("airports", decompressZstd(airportsRaw),
		...
		func(s []string) {
			...
			ap := Airport{Id: s[3], Name: s[4], Location: loc, Elevation: int(elevation)}
			if ap.Id != "" {
				airports[ap.Id] = ap
			}
		})
...
func parsePRD() map[AirportPair][]PRDEntry {
	...
	mungeCSV("prd", decompressZstd(prdRaw),
		[]string{"Orig", "Route String", "Dest", "Hours1", "Hours2", "Hours3", "Type", "Area", "Altitude",
...
func parseCallsigns() map[string]Callsign {
	...
	mungeCSV("virtual callsigns", decompressZstd(virtualCallsignsRaw),
		[]string{"COMPANY", "COUNTRY", "TELEPHONY", "3 LETTER"},
		addCallsign)
...
func (s *StaticDrawConfig) Draw(ctx *PaneContext, labelFont *Font, color *RGB,
	...
	*/
	...
	if s.DrawVORNames {
		drawloc(s.DrawEverything || s.DrawVORs, s.VORsToDraw, database.VORs, filterColor(ctx.cs.VOR), DrawLeft)
	}
	if s.DrawNDBNames {
		drawloc(s.DrawEverything || s.DrawNDBs, s.NDBsToDraw, database.NDBs, filterColor(ctx.cs.NDB), DrawLeft)
	}
	if s.DrawFixNames {
		drawloc(s.DrawEverything || s.DrawFixes, s.FixesToDraw, database.fixes, filterColor(ctx.cs.Fix), DrawRight)
	}
	if s.DrawAirportNames {
		...
		drawloc(s.DrawEverything || s.DrawAirports, s.AirportsToDraw, locs, filterColor(ctx.cs.Airport), DrawBelow)
	}
